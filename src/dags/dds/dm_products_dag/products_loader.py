            ...
            cur.execute(
                """
                    WITH t1 AS (
                        select 
                            oo.id,
                            -- ((object_value::jsonb ->> 'restaurant')::jsonb ->> 'id') as restaurant_id,
                            dr.id as restaurant_id,
                            (object_value::jsonb ->> 'order_items') as order_items,
                    ...
                    select
                        t2.id,
                        t2.restaurant_id,
                        t2.obj::json ->> 'id' as product_id,
                        t2.obj::json ->> 'name' as product_name,
                        t2.obj::json ->> 'price' as product_price,
                        t2.active_from,
                        t2.active_to
                    from (
                        SELECT 
                            ...
                            jsonb_array_elements_text(order_items::jsonb) AS obj 
                        FROM t1
                    ) as t2
